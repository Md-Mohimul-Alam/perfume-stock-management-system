  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        // Map columns
        const nameCol = findCol(['product name', 'name']);
        const skuCol = findCol(['sku', 'code']);
        const sizeCol = findCol(['size', 'sizeml', 'ml']);
        const priceCol = findCol(['price', 'sellingprice', 'selling price']);
        ...
        const notesCol = findCol(['notes', 'scent notes']);
        const bestsellerCol = findCol(['bestseller', 'isbestseller']);
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            ...
            if (!name || !sku || isNaN(price) || price < 0 || !sizeInfo) {
              errors.push(`Row ${i+2}: Missing or invalid data (name, sku, price, size)`);
              continue;
            }
            ...
            const item = {
              ...
              bestFor: bestForCol ? String(row[bestForCol]).split(',').map(s => s.trim()).filter(Boolean) : ['all'],
              notes: notesCol ? String(row[notesCol]).split(',').map(s => s.trim()).filter(Boolean) : [],
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        {uploadResult.errors?.length > 0 && (
                          <details className="mt-1">
                            <summary className="cursor-pointer text-sm">
                              View parse warnings ({uploadResult.errors.length})
                            </summary>
                            <ul className="text-xs mt-1 space-y-1 max-h-40 overflow-y-auto">
