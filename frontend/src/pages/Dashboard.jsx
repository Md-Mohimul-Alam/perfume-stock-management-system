...
import {
  ...
  Clock, Trash2, RefreshCw, RotateCw,
} from 'lucide-react';
import API from '../api/axios';
import toast from 'react-hot-toast';

// Custom CSS for ultra‑wide screens (27″ and beyond)
...
const Dashboard = () => {
  ...
  const overallSummary = [
    { label: 'Total Revenue', value: `৳${stats.totalRevenue.toFixed(2)}`, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50/50' },
    { label: 'Total Expenses', value: `৳${stats.totalExpenses.toFixed(2)}`, icon: Wallet, color: 'text-rose-600', bg: 'bg-rose-50/50' },
    { label: 'Total Purchases', value: `৳${stats.totalPurchases.toFixed(2)}`, icon: ShoppingCart, color: 'text-orange-600', bg: 'bg-orange-50/50' },
    { label: 'Pending Payments', value: `৳${stats.dueAmount.toFixed(2)}`, icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-50/50', badge: `${stats.dueCount} due`, link: '/sales?paymentStatus=due', linkText: 'View due →' },
    ...
    { label: 'Total Business Value', value: `৳${(stats.netProfit + stats.totalInventoryValue).toFixed(2)}`, icon: BarChart3, color: 'text-amber-600', bg: 'bg-amber-50/50' },
  ...
  return (
    <>
      ...
      <div className="dashboard-container p-4 sm:p-6 space-y-6">
        ...
        {/* Header */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600/5 via-purple-600/5 to-amber-600/5 border border-gray-200/50 p-4 sm:p-6 lg:p-8">
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 text-xs sm:text-sm mt-1 flex items-center gap-2 flex-wrap">
                <Calendar size={16} className="text-indigo-400" />
                <span>{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
              </p>
            </div>
            <div className="flex flex-wrap gap-2 sm:gap-3">
              <Link to="/sales/new" className="inline-flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 shadow-amber-500/25 text-xs sm:text-sm font-medium">
                <TrendingUp size={18} />
                <span>New Sale</span>
              </Link>
              <Link to="/expenses" className="inline-flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-rose-500 to-rose-600 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 shadow-rose-500/25 text-xs sm:text-sm font-medium">
                <Wallet size={18} />
                <span>Add Expense</span>
              </Link>
              <button onClick={() => navigate('/wastage/new')} className="inline-flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-red-500 to-red-600 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 shadow-red-500/25 text-xs sm:text-sm font-medium">
                <Trash2 size={18} />
                <span>Wastage</span>
              </button>
              {user?.role === 'admin' && (
                <button onClick={handleRebuild} disabled={rebuilding} className="inline-flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 shadow-purple-500/25 text-xs sm:text-sm font-medium disabled:opacity-60 disabled:cursor-not-allowed">
                  <RotateCw size={18} className={rebuilding ? 'animate-spin' : ''} />
                  <span>{rebuilding ? 'Rebuilding...' : 'Rebuild Stock'}</span>
                </button>
              )}
              <button onClick={fetchDashboardData} className="inline-flex items-center gap-1.5 sm:gap-2 bg-gray-200 text-gray-700 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl hover:bg-gray-300 transition text-xs sm:text-sm font-medium">
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-3 sm:gap-4">
              {mainCards.map((card, idx) => (
                <div key={idx} className={`bg-white rounded-2xl border ${card.border} hover:shadow-lg transition-all duration-300 hover:scale-[1.02] p-3 sm:p-4 stat-card`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-8 gap-3 sm:gap-4">
                {overallSummary.map((item, idx) => (
                  <div key={idx} className="bg-white rounded-2xl border border-gray-200/60 hover:shadow-md transition-all duration-200 p-3 sm:p-4 text-center relative">
            ...
            {/* Two-Column: Recent Expenses & Purchases */}
